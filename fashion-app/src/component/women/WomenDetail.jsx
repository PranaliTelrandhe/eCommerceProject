...
import womenProducts from "../women/women_data";

const WomenDetail = () => {
  const { prod_id } = useParams();
  const navigate = useNavigate();

  const product = womenProducts.find((item) => item.id == prod_id);
  ...
  return (
    <div className="d-flex justify-content-around">
      ...
      <div className="d-flex flex-column justify-content-center w-100 gap-4">
        ...
        <div>
          <button className="btn btn-outline-secondary" onClick={() => navigate("/women")}>
            Back to Gallery
          </button>
        </div>
      </div>
